...
final class UpdateStaffRequest extends FormRequest
{
    public function authorize(): bool
    {
        ...
        return $membership instanceof Membership && app(PermissionResolver::class)->allows($membership, 'staff.update');
    }
    ...
    public function rules(): array
    {
        ...
        $staffId = $staff instanceof StaffProfile ? $staff->getKey() : $staff;
        $profile = StaffProfile::query()->where('organization_id', $organization?->getKey())->find($staffId);

        return [
            'employee_number' => ['required', 'string', 'max:100', Rule::unique('staff_profiles')->where('organization_id', $organization?->getKey())->ignore($staffId)],
            'title' => ['nullable', 'string', 'max:50'],
            'first_name' => ['required', 'string', 'max:100'],
            'last_name' => ['required', 'string', 'max:100'],
            'email' => ['required', 'email:rfc', 'max:255', Rule::unique('users', 'email')->ignore($profile?->getAttribute('user_id'))],
