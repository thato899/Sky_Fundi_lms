...
The active organization always comes from trusted web-session context. Learner UUIDs are resolved inside that organization, foreign academic values are rejected, and action links/forms are shown only for the applicable `learners.*` permission. Manual learner numbers appear only with `learners.override_number`; creation otherwise uses `LearnerNumberService`. Profile creation itself never creates a User or Membership and leaves portal access disabled — see below for how an administrator-sent invitation changes that.
...
Learner onboarding mirrors the guardian flow field-for-field via `LearnerInvitationService`: only an active learner profile with an email is invitable; `learners.invite` gates send/resend/revoke (revoke reuses the same permission — there is no separate learner revoke permission); the same seven-day, SHA-256-hashed, single-use token discipline applies; acceptance creates a new User only when none matches the invited email, otherwise requires signing in as that exact account; and successful acceptance transactionally activates the membership, links the learner profile's `user_id`/`organization_membership_id`, sets `portal_access_enabled = true`, and marks `onboarding_status = completed`. The accepted learner lands on their own quiz assignment page. The learner profile's web page shows the pending invitation's status/expiry with resend/revoke actions once one exists, mirroring the guardian profile page.
...
- **Testing:** Unit/Feature files cover schema, service behavior, numbering, status, enrolment history, directory/API/web management, guardian and learner invitation/onboarding (happy path, resend/rotate, revoke/expired/reused tokens, new/existing-account acceptance, cross-organization isolation), guardian billing-portal visibility, isolation, and regressions.
