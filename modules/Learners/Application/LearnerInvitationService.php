...
final class LearnerInvitationService
{
    ...
    public function invite(LearnerProfile $learner, User $actor, string $email): array
    {
        $email = Str::lower(trim($email));
        $this->assertLearnerIsInvitable($learner);

        $result = DB::transaction(function () use ($learner, $actor, $email): array {
            /** @var LearnerProfile $lockedLearner */
            $lockedLearner = LearnerProfile::query()->whereKey($learner->getKey())->lockForUpdate()->firstOrFail();
            $this->assertLearnerIsInvitable($lockedLearner);

            /** @var Membership|null $existing */
            $existing = $lockedLearner->organization_membership_id === null
                ? null
                : Membership::query()->whereKey($lockedLearner->organization_membership_id)->lockForUpdate()->first();
            if ($existing instanceof Membership && $existing->status === MembershipStatus::Invited && ! $existing->invitation_expires_at?->isPast()) {
                ...
                return $this->rotateLocked($existing, $lockedLearner, $actor, false);
            }

            if ($existing instanceof Membership && $existing->status === MembershipStatus::Active) {
                throw new DomainException('This learner already has active portal access.');
            }
            ...
            $membership = Membership::query()
                ->where('organization_id', $lockedLearner->organization_id)
            ...
            if ($membership instanceof Membership && $membership->status === MembershipStatus::Active) {
                throw new DomainException('This email already has active organization access. Link that membership to the learner instead.');
            }

            /** @var Role $role */
            $role = Role::query()->where('name', 'Learner')->firstOrFail();
            $membership ??= new Membership;
            $membership->fill([
                'user_id' => $user?->getKey(),
                'organization_id' => $lockedLearner->organization_id,
                'role_id' => $role->getKey(),
            ...
            return $this->rotateLocked($membership, $lockedLearner, $actor, true);
        }, 3);

        $this->send($result['membership'], $learner, $result['token']);

        return $result;
    }

    /** @return array{membership: Membership, token: string} */
    public function resend(Membership $membership, LearnerProfile $learner, User $actor): array
    {
        $result = DB::transaction(function () use ($membership, $learner, $actor): array {
            ...
            return $this->rotateLocked($locked, $learner, $actor, false);
        }, 3);
    ...
    public function revoke(Membership $membership, LearnerProfile $learner): Membership
    {
        return DB::transaction(function () use ($membership, $learner): Membership {
            /** @var Membership $locked */
            $locked = Membership::query()->whereKey($membership->getKey())->lockForUpdate()->firstOrFail();
            if ($locked->status !== MembershipStatus::Invited) {
            ...
            $this->audit->record('learners.invitation_revoked', $learner, after: ['email' => $locked->invited_email]);
    ...
    public function accept(string $token, ?User $authenticatedUser, array $data): LearnerProfile
    {
        return DB::transaction(function () use ($token, $authenticatedUser, $data): LearnerProfile {
            $membership = $this->resolve($token, true);
            /** @var LearnerProfile|null $learner */
            ...
            $this->linkIdentity($learner, $membership, $user);
            $this->audit->record('learners.invitation_accepted', $learner, after: ['email' => $email]);
            $this->audit->record('learners.identity_linked', $learner, after: ['membership_status' => 'active']);
    ...
    /** @return array{membership: Membership, token: string} */
    private function rotateLocked(Membership $membership, LearnerProfile $learner, User $actor, bool $created): array
    {
        $token = Str::random(64);
        $membership->forceFill([
            'status' => MembershipStatus::Invited,
        ...
        $learner->forceFill([
            'learner_email' => $membership->invited_email,
            'organization_membership_id' => $membership->getKey(),
            'user_id' => null,
            'updated_by' => $actor->getKey(),
        ])->save();
        $this->audit->record(
            $created ? 'learners.invitation_created' : 'learners.invitation_resent',
            $learner,
    ...
    private function send(Membership $membership, LearnerProfile $learner, string $token): void
    {
        /** @var Organization $organization */
        $organization = $learner->organization()->firstOrFail();
        $this->notifications->sendToEmail((string) $membership->invited_email, 'Learner portal invitation', [
            'message' => "{$organization->name} invited you to its learner portal. This link expires {$membership->invitation_expires_at?->toDayDateTimeString()}. No learner information is included in this email.",
            'action_text' => 'Review invitation',
            'action_url' => route('learner-invitations.show', ['token' => $token]),
        ]);
    }

    private function linkIdentity(LearnerProfile $learner, Membership $membership, ?User $user): void
    {
        $duplicate = LearnerProfile::query()
            ->where('organization_id', $learner->organization_id)
            ->where('organization_membership_id', $membership->getKey())
            ->whereKeyNot($learner->getKey())
            ->exists();
        if ($duplicate) {
            throw new DomainException('This organization membership is already linked to another learner.');
        }
        $learner->forceFill([
            'organization_membership_id' => $membership->getKey(),
            'user_id' => $user?->getKey(),
    ...
    private function assertLearnerIsInvitable(LearnerProfile $learner): void
    {
        if ($learner->learner_status !== LearnerStatus::Active || $learner->archived_at !== null || $learner->deleted_at !== null) {
            throw new DomainException('Only active learner profiles may be invited.');
        }
    }
}
