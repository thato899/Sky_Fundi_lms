...
final class AcademicYearRolloverService
{
    ...
    public function override(EnterpriseOperationRun $run, User $actor, string $learnerId, array $override): EnterpriseOperationRun
    {
        ...
        foreach ($learners as &$outcome) {
            if (($outcome['learner_id'] ?? null) !== $learnerId) { continue; }
            ...
            if ($grade === null) { throw new DomainException('A manual placement requires an active destination grade in this organization.'); }
    ...
    public function submit(EnterpriseOperationRun $run, User $actor): EnterpriseOperationRun
    {
        ...
        if (($summary['blocking_errors'] ?? 0) > 0) { throw new DomainException('Resolve all blocking errors before submitting this rollover.'); }
    ...
    {
        if ($run->getAttribute('requested_by') === $actor->getKey()) { throw new DomainException('The requester may not approve an academic-year rollover.'); }
    ...
    public function execute(EnterpriseOperationRun $run, User $actor): EnterpriseOperationRun
    {
        ...
        return DB::transaction(function () use ($run, $actor): EnterpriseOperationRun {
            ...
            $this->assertType($run, ['approved']);
            $preview = $run->getAttribute('preview') ?? [];
            if (($preview['summary']['blocking_errors'] ?? 0) > 0) { throw new DomainException('This rollover contains unresolved blocking errors.'); }
            $run->update(['status' => 'executing']);
            $created = [];
            foreach ($preview['learners'] ?? [] as $outcome) {
                if (! in_array($outcome['status'] ?? '', ['ready', 'warning'], true)) { continue; }
                $learner = LearnerProfile::query()->withoutGlobalScopes()->lockForUpdate()->findOrFail($outcome['learner_id']);
                if ($learner->getAttribute('organization_id') !== $run->getAttribute('organization_id')) { throw new DomainException('Tenant boundary violation.'); }
                if (LearnerEnrolment::query()->where('learner_profile_id', $learner->getKey())->where('academic_year_id', $preview['destination_year_id'])->exists()) { throw new DomainException('A destination enrolment already exists for '.$learner->getKey().'.'); }
                $source = LearnerEnrolment::query()->where('learner_profile_id', $learner->getKey())->whereNull('ended_on')->lockForUpdate()->first();
                if ($source !== null) { $source->update(['ended_on' => now()->toDateString()]); }
            ...
            if (($run->getAttribute('payload')['activate_destination'] ?? false) === true) { $this->academicYears->setCurrent(AcademicYear::query()->withoutGlobalScopes()->findOrFail($preview['destination_year_id'])); }
    ...
    public function rollback(EnterpriseOperationRun $run, User $actor): EnterpriseOperationRun
    {
        ...
        return DB::transaction(function () use ($run, $actor): EnterpriseOperationRun {
            ...
            foreach ($generated as $row) {
                if ($row->getAttribute('updated_at')->greaterThan($run->getAttribute('executed_at')) || LearnerProfile::query()->withoutGlobalScopes()->find($row->getAttribute('learner_profile_id'))?->getAttribute('current_academic_year_id') !== $row->getAttribute('academic_year_id')) { throw new DomainException('Rollback is blocked because a generated placement has subsequent changes.'); }
            }
            foreach ($generated as $row) {
    ...
    private function years(Organization $organization, array $configuration): array { $source = AcademicYear::query()->withoutGlobalScopes()->where('organization_id', $organization->getKey())->find($configuration['source_year_id'] ?? null); $destination = AcademicYear::query()->withoutGlobalScopes()->where('organization_id', $organization->getKey())->find($configuration['destination_year_id'] ?? null); if ($source === null || $destination === null || $source->is($destination)) { throw new DomainException('Select two different academic years belonging to this organization.'); } return [$source, $destination]; }
    /** @return array<string, mixed> */
    private function propose(LearnerProfile $learner, AcademicYear $source, AcademicYear $destination, array $configuration): array { $current = $this->grade($learner->getAttribute('organization_id'), $learner->getAttribute('current_grade_id')); $rules = $configuration['rules'] ?? []; $rule = $rules[$current?->getKey()] ?? null; $proposedId = is_array($rule) ? ($rule['destination_grade_id'] ?? null) : $rule; $retained = is_array($rule) && (($rule['action'] ?? null) === 'retain'); if ($retained) { $proposedId = $current?->getKey(); } $grade = $this->grade($learner->getAttribute('organization_id'), $proposedId); $class = $grade === null ? null : ClassGroup::query()->withoutGlobalScopes()->where('organization_id', $learner->getAttribute('organization_id'))->where('academic_year_id', $destination->getKey())->where('grade_id', $grade->getKey())->where('status', 'active')->orderBy('name')->first(); $status = $current === null || ($grade === null && ! $retained) ? 'blocked' : ($class === null ? 'warning' : 'ready'); if ($rule === 'complete' || (is_array($rule) && ($rule['action'] ?? null) === 'complete')) { $status = 'completing'; $grade = null; $class = null; } return ['learner_id' => $learner->getKey(), 'learner_name' => trim($learner->getAttribute('first_name').' '.$learner->getAttribute('last_name')), 'current' => ['year_id' => $source->getKey(), 'grade_id' => $current?->getKey(), 'class_id' => $learner->getAttribute('current_class_id')], 'proposed' => ['grade_id' => $grade?->getKey(), 'class_id' => $class?->getKey(), 'curriculum_id' => $grade?->getAttribute('curriculum_id')], 'status' => $status, 'reason' => $status === 'blocked' ? 'A valid destination grade is required.' : ($status === 'warning' ? 'Destination class has not been assigned.' : null), 'retained' => $retained]; }
    private function grade(string $organizationId, ?string $id): ?Grade { return $id === null ? null : Grade::query()->withoutGlobalScopes()->where('organization_id', $organizationId)->where('status', 'active')->find($id); }
    private function class(string $organizationId, ?string $id, ?string $gradeId): ?ClassGroup { return $id === null ? null : ClassGroup::query()->withoutGlobalScopes()->where('organization_id', $organizationId)->where('status', 'active')->when($gradeId, fn ($query) => $query->where('grade_id', $gradeId))->find($id); }
    /** @param array<int, array<string, mixed>> $outcomes @return array<string, int> */
    private function summary(array $outcomes): array { $counts = ['total' => count($outcomes), 'ready' => 0, 'warnings' => 0, 'blocking_errors' => 0, 'manual_review' => 0, 'retained' => 0, 'completing' => 0]; foreach ($outcomes as $outcome) { $status = $outcome['status'] ?? ''; if ($status === 'ready') { ++$counts['ready']; } if ($status === 'warning') { ++$counts['warnings']; } if ($status === 'blocked') { ++$counts['blocking_errors']; ++$counts['manual_review']; } if ($status === 'completing') { ++$counts['completing']; } if (($outcome['retained'] ?? false) === true) { ++$counts['retained']; } } return $counts; }
    /** @param list<string> $allowed */ private function assertType(EnterpriseOperationRun $run, array $allowed): void { if ($run->getAttribute('operation_type') !== self::TYPE || ! in_array($run->getAttribute('status'), $allowed, true)) { throw new DomainException('This rollover is not in a valid workflow state for that action.'); } }
}
