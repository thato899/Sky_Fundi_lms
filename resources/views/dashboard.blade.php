...
<div class="wrap dashboard">
    ...
    <section class="section" aria-labelledby="management-heading"><h2 id="management-heading">Management areas</h2><div class="manage-grid">
        ...
        @if(in_array('quizzes.view', $permissions ?? [], true))<article class="feature"><h3>AI-assisted quizzes</h3><p>Create quizzes, review submissions, approve AI suggestions, and release study plans.</p><a href="{{ route('assessments.index') }}">Open quizzes</a></article>@endif
        @if(in_array('subscriptions.view', $permissions ?? [], true))<article class="feature"><h3>Subscription & profitability</h3><p>Review plan limits, estimated AI cost, and demonstration contribution margin.</p><a href="{{ route('subscription.dashboard') }}">Open subscription dashboard</a></article>@endif
        @if(in_array('core.billing.manage', $permissions ?? [], true))<article class="feature"><h3>Billing</h3><p>Subscribe to a real plan and review usage-linked invoices and payment status.</p><a href="{{ route('billing.dashboard') }}">Open billing</a></article>@endif
        ...
        @if(in_array('scheduling.view', $permissions ?? [], true))<article class="feature"><h3>Scheduling</h3><p>Manage factual timetables, lessons, rooms, closures, and attendance-session links.</p><a href="{{ route('scheduling.dashboard') }}">Open scheduling</a></article>@else<article class="feature disabled"><h3>Scheduling</h3><p>You do not have permission to view scheduling.</p></article>@endif
    </div></section>
</div>
@endsection
