...
<div class="nav-side">@if(!empty($navigation['persona']))<span class="persona-chip">{{ $navigation['persona'] }}</span>@endif<a href="{{ route('security.two-factor.edit') }}" class="button secondary small">Security</a><form method="POST" action="{{ route('logout') }}">@csrf<button type="submit" class="button secondary small">Log out</button></form></div>
