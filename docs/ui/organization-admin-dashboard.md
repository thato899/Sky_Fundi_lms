...
Authorized users see a working Attendance management card when `attendance.view` is effective for the active organization. Otherwise it is permission-safe and disabled; Learner, Staff, Guardian (when installed), and Academic links are preserved.
...
The dashboard links to Staff management for memberships with `staff.view`, Learner management for memberships with `learners.view`, and Academic management for memberships with `academics.academic-years.view`. Users without the academic permission see a permission-safe disabled card.
