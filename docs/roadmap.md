...
- A known authorization inconsistency: the `/staff` API routes (staff CRUD, per-staff teaching-assignment endpoints) authorize via the platform-wide `permission:` middleware (`Core\RBAC`, direct user-to-role assignment), not `Core\Identity\Application\PermissionResolver` (organization-membership-scoped roles) used everywhere else in the codebase — an ordinary org-scoped "Organization Administrator" likely cannot call those specific endpoints today. The new organization-wide teaching-assignment routes added alongside this use the correct pattern; the pre-existing `/staff` routes have not been changed yet.
