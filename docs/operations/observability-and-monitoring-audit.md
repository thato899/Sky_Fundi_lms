...
| Confirmed observability gap | Request completion/latency: only successful API traffic was logged, with wall-clock timing. | API middleware logged method/path/status/duration and analytics. | One global routed-request log now uses monotonic timing, safe route template/name, status, actor/tenant UUID context, and a configurable slow warning. API analytics remains API-only. Streamed responses are excluded from slow warnings. Fatal failures that never return through middleware require runtime monitoring. |
...
| Confirmed observability gap | Deployment verification was fragmented. | `make health`, `migrate:status`, scheduled health command. | Read-only `platform:diagnose` reports environment/debug/key presence, migration-status availability, and readiness statuses without values or repair. It fails for unsafe production debug/key state, migration failure, or unhealthy readiness. Run it in CI/deployment with real configuration. |
...
| Pending migrations/bad production config | `platform:diagnose`, `migrate:status` | Release/configuration gate |
...
log gaps for failures outside the HTTP middleware response path.
