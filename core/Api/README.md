...
- `Http/Middleware/LogApiRequests` — appended globally. Writes one structured `PlatformLogger` completion line per routed request and one `Core\Analytics` `ApiRequest` counter event for API requests only. Never fails the response — logging/metrics errors are swallowed.
