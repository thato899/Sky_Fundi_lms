...
**MFA status**: API login enforces a confirmed TOTP factor through the optional `two_factor_code` field when the account has MFA enabled. Trusted devices are a separate, opt-in security control and must not be represented as equivalent to a second factor. Web challenge UX and administrator-wide enforcement remain follow-up work. See [Platform hardening stages 1–4](../../docs/security/platform-hardening-stages-1-4.md).
