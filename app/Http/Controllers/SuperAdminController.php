...
final class SuperAdminController
{
    ...
    {
        return view('super-admin.organizations', ['organizations' => Organization::latest()->paginate(20)]);
    }

    public function wizard(): View
    {
        return view('super-admin.wizard');
    }

    public function users(): View
    {
        return view('super-admin.table', ['title' => 'Users', 'rows' => User::latest()->paginate(20), 'columns' => ['name', 'email', 'status', 'created_at']]);
    }

    public function roles(): View
    {
        return view('super-admin.table', ['title' => 'Roles', 'rows' => Role::withCount('permissions', 'users')->paginate(20), 'columns' => ['name', 'description', 'is_system']]);
    }

    public function modules(): View
    {
        return view('super-admin.table', ['title' => 'Installed modules', 'rows' => ModuleRegistration::paginate(20), 'columns' => ['display_name', 'version', 'status', 'description']]);
    }

    public function ai(ProviderRegistry $providers): View
    {
        return view('super-admin.ai', ['providers' => $providers->all()]);
    }

    public function audit(): View
    {
        return view('super-admin.table', ['title' => 'Audit centre', 'rows' => AuditLog::with('actor')->latest()->paginate(50), 'columns' => ['action', 'actor_id', 'created_at']]);
    }

    public function health(HealthCheckManager $health): View
    {
        $checks = $health->runAll();

        return view('super-admin.health', ['checks' => $checks, 'overall' => $health->overallStatus($checks)->value]);
    }
}
